...
public class GUIGameScreenController {
    ...
    public void updateScreen(UpdateGameEvent updateGameEvent){
        ...
        for (WindowCard w: updateGameEvent.getWindowCardList()) {
            if (updateGameEvent.getUser().get(userN).equals(guiView.getUser()))
                showMyWindow(w, myWindow);
            else
                showTheirWindow(w);
            userN++;
        }
    ...
    private void showRoundTrack(List<RoundCell> roundTrack) {
    }

    private void showTheirWindow(WindowCard w) {
    }

    private void showMyWindow(WindowCard w, GridPane location) {
